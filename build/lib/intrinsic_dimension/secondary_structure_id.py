...
def secondary_structure_id(topology= None, trajectory=None, mol = None, simplified=True, projection_method = 'Distances', id_method = 'local', projection_kwargs = None, id_kwargs = None, verbose=True):
    '''
    Computes intrinsic dimension (ID) estimation on contiguous secondary structure elements (e.g., helices, strands, loops) identified from a protein trajectory.
    ...
    projection_method : str or callable, default='Distances'
        Method for generating the molecular projection. Can be one of:
            ...
            - 'Dihedrals': backbone or side-chain dihedral angles.
            - MoleculeKit metric class (check README for availability).
